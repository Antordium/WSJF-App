...
import { Trash2, PlusCircle, HelpCircle, Database, Download, FileText } from 'lucide-react';
import { exportToPDF, validateExportData, type RankedInitiative } from './utils/pdfExport';
...
const ExportButton = ({ onExport, disabled, isExporting }: ExportButtonProps) => (
  <button
    onClick={onExport}
    ...
    className={`inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg transition-colors ${
      disabled || isExporting
        ...
        : 'bg-green-600 hover:bg-green-700 text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-green-500'
    }`}
    title={disabled ? 'Add initiatives to enable PDF export' : 'Export prioritization to PDF'}
  >
    {isExporting ? (
      <>
        <div className="animate-spin w-4 h-4 mr-2 border-2 border-gray-300 border-t-transparent rounded-full"></div>
        Generating PDF...
      </>
    ) : (
      <>
...
function WSJFApp() {
  ...
  // --- Tooltip Content ---
  const tooltips = {
    uv: 'User Value / Training Readiness Impact: How
